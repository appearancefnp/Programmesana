...
void drawAstroid(float size, sf::RenderWindow &window) {
    ...
    for (float a = 0; a <= 2*PI; a += 0.0001) {
        //Aprekina astroida malas punktu
        ...
        //Uzstada kvadrantiem krasas
        if (a < PI/2)lines[0].color  = sf::Color::White;
        if (a > PI/2 and a < PI) lines[0].color  = sf::Color::Red;
        if (a > PI and a < PI*3/2) lines[0].color  = sf::Color::White;
        if (a > PI*3/2 and a < 2*PI) lines[0].color  = sf::Color::Red;
        window.draw(lines);
    }
}
